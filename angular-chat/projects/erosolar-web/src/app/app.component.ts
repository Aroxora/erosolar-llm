    ...
    <div class="page">
      ...
      <header class="hero">
        ...
        <p class="model-status"
           [class.live]="modelOnline() === true"
           [class.offline]="modelOnline() === false">
          <span class="status-dot"></span>
          <ng-container *ngIf="modelOnline() === null">connecting to the hosted model&hellip;</ng-container>
          <ng-container *ngIf="modelOnline() === true">live &mdash; generating from the hosted neural model on Cloud Run</ng-container>
          <ng-container *ngIf="modelOnline() === false">offline &mdash; using the in-browser template (the hosted model may be asleep; it wakes on first request)</ng-container>
        </p>
      </header>
        ...
        <section class="results" aria-live="polite">
          ...
          <article class="card" *ngFor="let c of cards()">
            <div class="card-head">
              ...
              <span class="card-src" [class.neural]="c.source === 'neural'"
                    [title]="c.source === 'neural' ? 'generated by the hosted neural model' : 'generated by the in-browser template fallback'">
                {{ c.source === 'neural' ? '⚡ model' : 'template' }}
              </span>
            </div>
...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  /** null = checking, true = live model API, false = offline template fallback. */
  modelOnline = signal<boolean | null>(null);
  ...
  ngOnInit(): void {
    ...
    // Probe the hosted model API (/api routes to Cloud Run via Firebase rewrite).
    this.http.get<ApiHealth>('/api/health').subscribe({
      next: (h) => this.modelOnline.set(h?.status === 'ok'),
      error: () => this.modelOnline.set(false),
    });
  ...
  /** Generate via the hosted neural model; fall back to the in-browser template. */
  private async makeAppreciation(quality: string): Promise<Card> {
    if (this.modelOnline() !== false) {
      try {
        ...
        this.modelOnline.set(false); // model asleep/unavailable — use the honest fallback
      }
    }
    const a = generateAppreciation(quality);
    return { ...a, id: this.nextId++, copied: false, source: 'template' };
  }
  ...
  async generate(): Promise<void> {
    ...
    this.busy.set(true);
    const card = await this.makeAppreciation(q);
  ...
  async regen(card: Card): Promise<void> {
    track('regenerate', { quality: card.quality });
    const fresh = await this.makeAppreciation(card.quality);
    this.cards.update((list) =>
      list.map((c) =>
        c.id === card.id ? { ...c, raw: fresh.raw, display: fresh.display, source: fresh.source, copied: false } : c,
      ),
    );
  }
